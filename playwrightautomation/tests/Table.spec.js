...
test('Table test', async ({ page }) => {
    ...
    const tableHeaders = await page.locator('table tbody tr th');
    const tableHeaderCount = await tableHeaders.count();
      const tableHeadersText = await tableHeaders.textContent();
      
    console.log("Total number of Table Headers: " + tableHeaderCount);
    console.log("Table Headers Text: " + tableHeadersText);

    const tableRows = await page.locator('tbody tr');
    const tableRowCount = await tableRows.count();
    console.log("Total number of Table Rows: " + tableRowCount);
